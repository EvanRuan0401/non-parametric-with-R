...
## how do lnhours effect lnwage (educ level at 12 years and locate in small city)
### fitted curve
...
data.eval <- data.frame(lnhours=seq(from=min(lnhours),to=max(lnhours),length=200),
                        ...
                        largecity=factor(rep(x=0,times=200)))
...
## how do educ effect lnwage (lnhours level at mean and locate in large city)
### fitted curve

data.eval <- data.frame(lnhours=rep(x=mean(x=lnhours),times=13),
...
## how do educ effect lnwage (lnhours level at mean and locate in small city)
### fitted curve

data.eval <- data.frame(lnhours=rep(x=mean(x=lnhours),times=13),
                        educ=ordered(sort(unique(educ))),
                        largecity=factor(rep(x=0,times=13)))
...
## how do city effect lnwage (lnhours level at mean and educ level at 12)
### fitted curve
data.eval <- data.frame(lnhours=rep(x=mean(x=lnhours,prob=0.5),times=2),
