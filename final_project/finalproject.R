...
data.eval <- data.frame(lnhours=seq(from=min(lnhours),to=max(lnhours),length=100),
                        educ=ordered(rep(x=16,times=100)),
...
data.eval <- data.frame(lnhours=rep(x=uocquantile(x=lnhours,prob=0.5),times=13),educ=ordered(sort(unique(educ))),largecity=factor(rep(x=1,times=13)))
...
plot(educ.star,fit,type="p",pch=16,col="black",xlab="Educ",ylab="Log(Wage)",main="LLLS (Cross-Validated Bandwidth Selector) and CI (Asymptotic Variance)",ylim=c(-5,10),lwd=2)
...
plot(educ.star,grad,type="p",pch=16,col="black",xlab="Years of Education",ylab="Marginal Effects",main="LLLS (Cross-Validated Bandwidth Selector)",ylim=c(-0.5,0.5),lwd=2)
...
data.eval <- data.frame(lnhours=rep(x=uocquantile(x=lnhours,prob=0.5),times=2),educ=ordered(rep(x=16,times=2)),largecity=factor(sort(unique(largecity))))
...
plot(largecity.star,fit,type="p",pch=16,col="black",xlab="CityLocation",ylab="Log(Wage)",main="LLLS (Cross-Validated Bandwidth Selector) and CI (Asymptotic Variance)",ylim=c(0,3),lwd=2)
...
plot(largecity.star,grad,type="p",pch=16,col="black",xlab="Housing Location",ylab="Marginal Effects",main="LLLS (Cross-Validated Bandwidth Selector)",ylim=c(-0.5,0.5),lwd=2)
